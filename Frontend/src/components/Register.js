  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      if (response.status === 409) {
        ...
        toast.error(msg || "Username or email already exists");
        return;
      }
      ...
      setTimeout(() => {
        if (user.roleId === 2) {
          // Mess Owner → MessCreationForm
          window.location.href = "/mess-creation";
        } else {
          // Customer → Homepage
      ...
      <form className="auth-form" onSubmit={handleSubmit(onSubmit)}>
        ...
        <input
          ...
          {...register("phone", {
            ...
            pattern: { value: /^[6-9]\d{9}$/, message: "10 digits starting with 6-9" }
          })}
        />
