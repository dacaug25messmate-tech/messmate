// corredefullyimport "../../styles/messformdisplay.css";
// import AddMessForm from "../../components/messowner/AddMessForm.jsx";
// import MonthlyPlans from "./MonthlyPlanList.js"; // Assuming you have this component
...
//                   setActiveTab("messDetails"); // go back to mess details
//                 }}
//               />
//             )}
//           </div>
...
export default function ProfileAndMessInfo() {
  ...
  // Load profile & mess data
  const loadProfile = () => {
    fetch(`http://localhost:2028/api/messowner/profile/${userId}`)
      .then((res) => res.json())
      .then(setData)
      .catch(() => setData(null));
  ...
  const handleDeleteMess = (messId) => {
    ...
    fetch(`http://localhost:2028/api/messowner/mess/${messId}`, {
      method: "DELETE",
    })
      .then((res) => {
        if (!res.ok) throw new Error();
...
//   // Sync tab with URL (optional if you want)
//   useEffect(() => {
//     if (location.pathname.endsWith("profile")) setActiveTab("profile");
//   }, [location.pathname]);
...
//     fetch(`http://localhost:2025/api/messowner/mess/${messId}`, { method: "DELETE" })
//       .then((res) => {
//         if (!res.ok) throw new Error();
//         alert("Mess deleted successfully");
//         loadProfile();
...
//         <button onClick={() => { if (!selectedMess) return alert("Select a mess first"); setActiveTab("addPhoto"); }} className={activeTab === "addPhoto" ? "tab-active" : ""}>
//           Add Photo
//         </button>
//       </div>
...
//           <table className="profile-table">
//             <thead>
//               <tr><th>Mess Name</th><th>Address</th><th>Action</th></tr>
//             </thead>
//             <tbody>
//               {myMesses.length === 0 ? (
...
//               <tr><th>Area</th><td>{selectedMess.areaId?.area_name || "N/A"}</td></tr>
//             </tbody>
//           </table>
//           <button onClick={() => setActiveTab("mess")}>Back to List</button>
//         </div>
...
//       {/* ADD / EDIT / UPLOAD PHOTO */}
//       {activeTab === "add" && <AddMessForm userId={userId} mode="add" onSaved={handleSaved} />}
//       {activeTab === "edit" && selectedMess && <AddMessForm userId={userId} mode="edit" messData={selectedMess} onSaved={handleSaved} />}
//       {activeTab === "addPhoto" && selectedMess && <UploadMessPhoto messId={selectedMess.messId} onUploaded={() => setActiveTab("messDetails")} />}
//     </div>
//   );
// }
// import "../../styles/messformdisplay.css";
...
// import MonthlyPlans from "./MonthlyPlanList.js";
// import UploadMessPhoto from "../../components/messowner/UploadMessPhoto.jsx";
// import MessList from "../../components/messowner/MessList.jsx";
// import { useEffect, useState } from "react";
...
//       .then((messes) => {
//         setMyMesses(messes);
//         if (messes.length > 0) setSelectedMess(messes[0]);
//       });
//   };
...
//               <button onClick={() => setActiveTab("mess")}>Mess Info</button>
//               <button onClick={() => setActiveTab("add")}>Add Mess</button>
//             </div>
...
//             {activeTab === "add" && (
//               <AddMessForm userId={userId} mode="add" onSaved={loadProfile} />
//             )}
//           </div>
//         )}
...
//         {activeMenu === "Manage Monthly Plans" && (
//           <div className="dashboard-card">
//             <MonthlyPlans selectedMess={selectedMess} />
//           </div>
//         )}
//       </div>
//     </div>
